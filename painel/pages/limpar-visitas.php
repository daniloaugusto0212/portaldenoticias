...
<div class="box-content">
    ...
    <form  method="post" > 
        ...
        if ($pegarVisitasTotais->rowCount() != 0) {
            $pegarVisitasTotais = $pegarVisitasTotais->fetch();
            $totalDasVisitas = $pegarVisitasTotais['total'];
        } else {
            $totalDasVisitas = 0;
        }
        $getVisists = $getVisistsWithDatas + $totalDasVisitas;

        $dateLastVisit = date('d/m/Y', strtotime($pegarVisitasTotais['ultima_limpeza']));
        echo "<h2>Última Limpeza em {$dateLastVisit}";
        ?>
